...
class LLK_Calculator:
    def __init__(self, data: pd.Series, compress_ratio=10.0, min_bins=100, lambda_pt=1.0, 
        ...
        self.x_percentile_1 = np.percentile(self.x, 1)
        self.x_percentile_10 = np.percentile(self.x, 10)
    ...
    def init_rv_from_gas_sn_class(self, g: GAS_SN):
        ...
        return self.init_rv(g.alpha, g.k, scale=g.scale)

    def init_rv(self, alpha: float, k: float, scale: float):
        self.alpha = float(alpha)
        self.k     = float(k)
        self.scale = float(scale)
        self.rv = self.get_rv(alpha, k, scale=scale)
        self._minus_log_likehood = None
        return self
    
    def get_rv(self, alpha: float, k: float, scale: float=1.0):
        if self.use_inverse is not None:
            if self.use_inverse == 'inv':
                return fcm_inverse(alpha, k, scale=scale)
            elif self.use_inverse == 'char':
                return frac_chi_mean(alpha, -k, scale=scale)
            else:
    ...
    def add_rv(self, lkc: 'LLK_Calculator', d_alpha=0.0, d_k=0.0, d_scale=0.0) -> 'LLK_Calculator':
        self.init_rv(lkc.alpha + d_alpha, lkc.k + d_k, lkc.scale + d_scale)
        return self
    ...
    def calc_mllk_with_regularization(self, debug=False, iter=None):
        ...
        B2 = self.lambda_pt * (self.rv.ppf(0.999)/self.x_percentile_999 - 1)**2 * 100 if self.use_999 else 0.0  # type: ignore
        C1 = self.lambda_pt * (self.rv.ppf(0.66)/self.x_percentile_66 - 1)**2 * 100 # type: ignore
        C2 = self.lambda_pt * (self.rv.ppf(0.20)/self.x_percentile_20 - 1)**2 * 10  # type: ignore
        C3 = self.lambda_pt * (self.rv.ppf(0.01)/self.x_percentile_1 - 1)**2 * 100  # type: ignore

        prefix = f"iter: {iter}, mllk-1" if iter is not None else "mllk-1"
        if debug: print(f"{prefix}: {A:.8f}, reg: ppf: 99 {B1:.8f} {B2:.8f} | {C1:.8f} {C2:.8f} {C3:.8f}")
    ...
    def calc_gradient(self):
        ...
        r = pd.Series({
            'alpha': self.alpha, 'k': self.k, 'scale': self.scale,
            'd_alpha': self.alpha * delta4,
            'd_k':     self.k * delta4,
            'd_scale': delta5,
            })

        def _calc_mllk(d_alpha=0.0, d_k=0.0, d_scale=0.0):
            try:
                self.init_rv(r['alpha'] + d_alpha, r['k'] + d_k, r['scale'] + d_scale)  # type: ignore
        ...
        self.init_rv(r.alpha, r.k, r.scale)  # type: ignore
        return r
...
class MLE:
    ...
    def get_allowed_param_space(self, param_space: pd.DataFrame):
        ...
        # df['loc'] = self.lkc.x_loc - df['mean']  # not sure what to do here
    ...
    def scan_param_space(self, param_space: pd.DataFrame, max_combo=1000):
        ...
        def _calc_mllk(r: pd.Series) -> float:
            self.lkc.init_rv(r.alpha, r.k, scale=r['scale'])  # type: ignore
    ...
    def gradient_descent(self, accept_max_iter=True) -> pd.Series:
        ...
        def _calc_gradient(iter: int) -> pd.Series:
            ...
            gr['iter'] = iter
            print(f"iter: {iter}, mllkc: {gr.mllk:.8f} / {gr.mllk_reg:.8f} from alpha: {gr.alpha:.6f}, k: {gr.k:.6f}, scale: {gr.scale:.6f}")
            return gr
...
class FCM_Px_Plot:
    ...
    def plot_pdf(self, ax, use_pdf=True, log_scale=False):
        ...
        if not use_pdf:
            ax.axhline(y=1 - 0.01, color='orange', linestyle='--', linewidth=0.5, label=f'{self.fcm_label} 1st pctile')
    ...
    def plot_4_panels(self, ax1, ax2, ax3, ax4):
        ...
        ax2.set_title(f'{self.fcm_label}: alpha={lkc.alpha:.3f}, k={lkc.k:.2f}, scale={lkc.scale:.2f}')
