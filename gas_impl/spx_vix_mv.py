...
class SPX_VIX_Data:
    def __init__(self, num_bins=200, end_date=pd.to_datetime('2024-03-31')):
        ...
        self.cov = np.cov(df.rtn_vix, y=df.rtn_spx)
    ...
    def hist2d_density(self):
        return np.histogram2d(self.ds(0), self.ds(1), bins=(self.num_bins, self.num_bins), density=True)

    @lru_cache(maxsize=10)
    def hist2d_peak_density(self, bins=4**2):
        counts = self.hist2d_density()[0]
        cnt = np.flip(np.sort(counts.flatten()))
        return (cnt[:bins]).mean()

    @lru_cache(maxsize=10)  # this is used a lot
    def hist1d_density(self, n: int):
        df = self.data_df
        return np.histogram(df[self.n2col(n)], bins=self.num_bins, density=True)
...
class SPX_VIX_Marginal_Dist:
    def __init__(self, data_obj: SPX_VIX_Data, length=40, model_name='gsas1'):
        ...
        self.grid_pos = np.dstack((self.grid_x, self.grid_y))  # 2d (x,y)
    ...
    @lru_cache(maxsize=10)
    def ell_peak_density(self): return float(self.ell_gsas.pdf(self.ell_gsas.x0))
    ...
    @lru_cache(maxsize=10)
    def adp_peak_density(self): return float(self.adp_gsas.pdf(self.adp_gsas.x0))
    ...
    def create_1d_pdf_df(self, n: int):
        ...
        df['p1'] = df['x'].parallel_apply(lambda x: g.pdf(x))
    ...
    def create_adp_mv_fn(self, cov): return Multivariate_GSaS_2D(cov=cov, alpha=self.alpha, k=self.k)
